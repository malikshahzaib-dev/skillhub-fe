...
const userId = userObj?._id
    ...
    const fetchApplication = async() => {
        try{
            const res = await api.get(`/application/organization/${userId}`)
            console.log("fetch all application successfully",res.data)
            setApplication(res.data)
        }catch(err:any){
            console.error("error to feth all application")
        }
    }
    useEffect(() =>{
        fetchApplication()
